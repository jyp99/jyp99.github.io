$(function(){
	...
	//요금계산기_리전선택
	$('.btn_region_select').click(function(e){
		$('.lst_select_region').show();
	});
	$('.region_select_area .link_region').click(function(e){
		$('.lst_select_region').hide();
	});	
	//예상 요금 내역 보기 버튼
	$('.btn_calculation_detail').click(function(e){
		$('.calculator_detail_wrap').addClass('is_show');
	});
	$('.btn_detail_close').click(function(e){
		$('.calculator_detail_wrap').removeClass('is_show');
	});

	$('.lst_region_price .btn_region').click(function(e){
		e.preventDefault();
		$(this).parents('.lst_region_price').find('.region_item').removeClass('is_selected');
		$(this).parents('.region_item').addClass('is_selected');
